...
/**
 * Applies a given stauts effect to this mob
 *
 * new_effect - TYPEPATH of a stauts effect to apply
 * Additional status effects arguments can be passed
 *
 * Returns the insance of the created effected, if successful
	...
	if(initial(new_effect.status_type) != STATUS_EFFECT_MULTIPLE)
		for(var/datum/status_effect/existing_effect as anything in status_effects)
			...
			switch(existing_effect.status_type)
				// Multple are allowed, continue as normal. (Not normally reachable)
