...
/datum/status_effect
	// The ID of the effect. ID is used in adding and removing effects to check for duplicated, among other things.
	var/id = "effect"
	// When set initially / on_creation, his is how long the status effect lasts in deciseconds
	// While processing, this becomes the world.time when the status effect will expire
	// -1 = infinite duration
	var/duration = -1
	// When set initially / on_creation, this is how long between [proc/tick] calls in deciseconds
	// Note that this cannot be faster than the processing subsystem you choose to fire the effect on
	...
	// How many of the effect can be one one mob, and/or wha happens whenyou try to add a duplcate
	var/status_type = STATUS_EFFECT_UNIQUE
	// If TRUE, we call [proc/on_remove] when owner is deleted. Otherwse, we call [proc/be_replaced]
	...
	// if TRUE, and we have alert, we wil show a duration on the alert
	...
	VAR_FINAL/obj/effect/effect/particle_effect
...
/datum/status_effect/proc/on_creation(mob/living/new_owner, ...)
	...
	if(alert_type)
		...
		// new_alert.attacked_effect = src
		linked_alert = new_alert
		update_shown_duration()

	if(duration > world.time || tick_interval > world.time)
 ...
 * If you're adding processed effects, put thme in [proc/tick]
...
// Gets and formats exmaine text associated with our status effect
 ...
 *
 * Note that every tick =/= ever processing cycle
 *
 * * seconds_between_ticks = This is how many SECONDS that elapse between ticks
 * This is a cosntant value based upon the initial tick interval set on the status effect
...
/**
 * Called whenver the buff expires or is removed (qdeleted)
 * Note that at the point this is called, it is out of the
 * owner's status_efects list, but owner is not yet null
 */
 ...
 * of status_type STTAUS_EFFECT_REPLACE is replaced by itself
...
// Adds nextmove modifier multiplicative to the owner while applied
...
/datum/status_effect/proc/remove_effect_on_heal(datum/soruce, heal_flags)
	SIGNAL_HANDLER
...
/**
 * Updated the particles for the status effects
